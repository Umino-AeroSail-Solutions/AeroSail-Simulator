...
# THIS IS DEFINETLY NOT A COPY OF Force_In_Rail_Calculator_Andres_saves_the_day
...
def get_reactions(P1, P2, P3, P4, l, m, h, draw=False, cogloc=h / 2):
    ...
    bottom_reaction_vector = np.array(([R1 * np.sin(alpha) + T * np.cos(alpha), R1 * np.cos(alpha)- T * np.sin(alpha)]))
    top_reaction_vector = np.array(([-R2 * np.sin(beta), R2 * np.cos(beta)]))
    axial_vector = C_vector / np.linalg.norm(C_vector)

    axial_force_top_vector = np.dot(top_reaction_vector, axial_vector)
    axial_force_top = np.linalg.norm(axial_force_top_vector)
    radial_force_top = np.linalg.norm(top_reaction_vector-axial_force_top_vector)

    axial_force_bottom_vector = np.dot(bottom_reaction_vector, axial_vector)
    axial_force_bottom = np.linalg.norm(axial_force_bottom_vector)
    radial_force_bottom = np.linalg.norm(bottom_reaction_vector-axial_force_bottom_vector)
    ...
    if draw:
        ...
        pygame.draw.line(screen, MateRed, (A + ((h / d) * (C_vector))) * scale + offset, (
                    (A + ((h / d) * (C_vector))) + [(w / d) * (D_vector[0]), (w / d) * (D_vector[1])]) * scale + offset,
                         1)
        # Top right corner sail
        pygame.draw.line(screen, MateRed, (A + ((h / d) * (C_vector))) * scale + offset, (
                    (A + ((h / d) * (C_vector))) - [(.398 / .2 * w / d) * (D_vector[0]),
                                                    (.398 / .2 * w / d) * (D_vector[1])]) * scale + offset, 1)
        ...
        draw_arrow(screen, (A + ((cogloc / d)) * (C_vector)) * scale + offset, (-np.pi / 2), m * 9.81 * vector_scale,
                   YELLOW)
    return axial_force_top, radial_force_top, axial_force_bottom, radial_force_bottom, B
...
for l in l_values:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
    axial_force_top, radial_force_top, axial_force_bottom, radial_force_bottom, B = get_reactions(P1, P2, P3, P4, l, m, h, draw=draw, cogloc=cogloc)
    a_f_top_values.append(axial_force_top)
    r_f_top_values.append(radial_force_top)
    a_f_bot_values.append(axial_force_bottom)
    r_f_bot_values.append(radial_force_bottom)
    if draw:
        # Update the display
        pygame.display.flip()
...
L_Top = max(l2_values)
L_Bot = max(l_values)
a_f_top_max = np.max(np.abs(np.array(a_f_top_values)))
r_f_top_max = np.max(np.abs(np.array(r_f_top_values)))
# print("a_f_top max: ", np.max(np.abs(np.array(a_f_top_values))))
# print("r_f_top max: ", np.max(np.abs(np.array(r_f_top_values))))
# print("a_f_bot_values_values max: ", np.max(np.abs(np.array(A_F_BOT_values))))

# Plot reaction forces as a function of l
plt.figure(figsize=(8, 5))
plt.plot(l_values, a_f_top_values, label='a_f_top', color='r')
plt.plot(l_values, r_f_top_values, label='r_f_top', color='g')
plt.plot(l_values, a_f_bot_values, label='a_f_bot', color='b')
plt.plot(l_values, r_f_bot_values, label='r_f_bot', color='black')
...
plt.figure(figsize=(8, 5))
plt.plot(l2_values, r_f_top_values, label='r_f_top', color='g')
...
while running:
    ...
    l = (mx) * (L_Bot / WIDTH)
    get_reactions(P1, P2, P3, P4, l, m, h, draw=True, cogloc=cogloc)
    pygame.display.flip()
