...
loadcombifactors = {
    ...
    "OST-1P": [0.1 * f_T - 0.15, 0.7 - 0.3 * f_T, 0.2 * f_T - 0.45, 0, 0.4 * f_T - 0.25, 0.1 - 0.2 * f_T,
               0.2 * f_T - 0.05, 0.4 * f_T - 0.25, 0.7 - 0.3 * f_T],
    "OST-2P": [-0.1 * f_T + 0.15, -0.7 + 0.3 * f_T, -0.2 * f_T + 0.45, 0, -0.4 * f_T + 0.25, -0.1 + 0.2 * f_T,
               -0.2 * f_T + 0.05, -0.4 * f_T + 0.25, -0.7 + 0.3 * f_T],
    "OST-1S": [0.1 * f_T - 0.15, 0.7 - 0.3 * f_T, 0.2 * f_T - 0.45, 0, -0.4 * f_T + 0.25, -0.1 + 0.2 * f_T,
               0.2 * f_T - 0.05, -0.4 * f_T + 0.25, 0.7 - 0.3 * f_T],
    "OST-2S": [-0.1 * f_T + 0.15, -0.7 + 0.3 * f_T, -0.2 * f_T + 0.45, 0, 0.4 * f_T - 0.25, 0.1 - 0.2 * f_T,
               -0.2 * f_T + 0.05, 0.4 * f_T - 0.25, -0.7 + 0.3 * f_T],
    ...
    "OSA-1S": [-0.45, 0.5, -0.8, 0.2 + 0.1 * f_T, -0.3 + 0.2 * f_T, -0.1 * f_T + 0.2, -0.2 * f_T, -0.3 + 0.2 * f_T,
               1.0],
...
def get_acc_dyn(x, y, z, LCF):
    a_x = f_beta * (
                - loadcombidf[LCF]["C_XG"] * g * np.sin(phi) + loadcombidf[LCF]["C_XS"] * a_surge + loadcombidf[LCF][
            "C_XP"] * a_pitch * (z - R))

    a_y = f_beta * (loadcombidf[LCF]["C_YG"] * g * np.sin(theta) + loadcombidf[LCF]["C_YS"] * a_sway - loadcombidf[LCF][
        "C_YR"] * a_roll * (z - R))
...
def get_acc_env(x, y, z):
    ...
    a_x_env = 0.7 * f_L * (0.65 + 2 * z / 7 / T_SC) * np.sqrt(
        a_surge ** 2 + L_0 / 325 * (g * np.sin(phi) + a_pitch * (z - R)) ** 2)

    a_y_env = (1 - np.exp(-B * L / 215 * GM)) * np.sqrt(a_sway ** 2 + (g * np.sin(theta) + a_roll * (z - R)) ** 2)

    a_z_env = np.sqrt(
        a_heave ** 2 + ((0.95 + np.exp(-L / 15)) * a_pitch * (1.08 * x - 0.45 * L)) ** 2 + (1.2 * a_roll * y) ** 2)
